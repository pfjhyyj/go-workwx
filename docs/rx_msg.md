...
const EventTypeKfAccountAuthChange = "kf_account_auth_change"
```
...
### `rxEventKfMsgOrEvent` 接受的事件消息，微信客服消息或事件
...
### `rxEventKfAccountAuthChange` 接受的事件消息，客服账号授权变更事件

Name|XML|Type|Doc
:---|:--|:---|:--
`AuthAddOpenKfId`|`AuthAddOpenKfId`| `[]string` |新增授权的客服账号列表，多个AuthAddOpenKfId节点表示多个新增账号
`AuthDelOpenKfId`|`AuthDelOpenKfId`| `[]string` |取消授权的客服账号列表，多个AuthDelOpenKfId节点表示多个取消账号
